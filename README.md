...
✅ Requirements for Premium Features
      ...
      Forum Topics require:
           1. Group converted to Forum in settings
           2. Bot promoted to Admin with Manage Topics permission
...
the main goal of this was to create a telegram bot that would stucture my comic collection how i want it to be done on telegram well uploading,
so picture a directory full of sub folder in each folder are comics with cover images (one can apply this to tv shows or movies or anime folder), the bot will create a topic of that folder name, it will first upload the image and then the data that is in that folder to the telegram u add the bot to.

1. get a telegram Bot Token from @BotFather, 
2. get the py and place it where u want, then edit it on line 8, where it says YOUR_BOT_TOKEN_HERE insert your token key there. 
(add the bot to where you want in telegram to make sure it has the correct premissions).
and then start the bot within the telegram group /start.
