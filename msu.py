...
from telegram import Update, InputMediaPhoto, ChatMemberAdministrator
...
async def upload_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Format status line
        status_line = f"📌 Topics: {'ON' if topics_enabled else 'OFF'} | 📦 Album: {'ON' if album_mode else 'OFF'} | 🖼️ AlbumCap: {'ON' if album_captions else 'OFF'} | 📝 Docs: {'ON' if doc_captions else 'OFF'} | 🖼️ ImgCap: {'ON' if image_captions else 'OFF'}"
...
def main():
    ...
    print("with album grouping, topics, and smart captions")
    print("="*60 + "\n")
